...
Specs:
  ...
  - Total max height (center crown):   15 mm (1.5 cm)
  - Top arc: 36" (914.4 mm) diameter wheel  ->  R = 457.2 mm
  - Sagitta (crown rise above edges): 11.07 mm
...
L = 200.0          # length (X)
W = 49.0           # width  (Y)
TOP_MAX = 15.0     # total max height at center (constraint)
R = 457.2          # 36" diameter / 2
SAG = R - np.sqrt(R**2 - (L/2)**2)   # ≈ 11.07 mm
BASE = TOP_MAX - SAG                  # edge thickness ≈ 3.93 mm

print(f"Sagitta:        {SAG:.3f} mm")
print(f"Edge thickness: {BASE:.3f} mm")
...
# Edge thickness
dim((-L/2-8, 0, 0), (-L/2-8, 0, BASE), f"Edge = {BASE:.2f} mm", offset=(-30,0,-2))
# Sagitta
dim((0, W+6, BASE), (0, W+6, BASE+SAG), f"Sagitta = 11.07 mm", offset=(0,8,2))
# Total max height
dim((L/2+8, W, 0), (L/2+8, W, BASE+SAG), f"Max H = 15.00 mm", offset=(15,0,4))
...
ax.set_title('Radius Platen — 36" wheel profile, 200 × 49 mm, 15 mm max height',
